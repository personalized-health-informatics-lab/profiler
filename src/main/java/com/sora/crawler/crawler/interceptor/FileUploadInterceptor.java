package com.sora.crawler.crawler.interceptor;
...
public class FileUploadInterceptor extends HandlerInterceptorAdapter {
    ...
    private static List<String> allowFileTypeList = new ArrayList<>(Arrays.asList("xlsx"));
    ...
    private void outputStream(HttpServletRequest request, HttpServletResponse response) throws IOException {
        response.setCharacterEncoding(request.getCharacterEncoding());
        ServletOutputStream output = null;
        try {
            output = response.getOutputStream();
            output.write(("The type of file can be uploaded are listed below: " + Arrays.toString(allowFileTypeList.toArray())).getBytes(request.getCharacterEncoding()));
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            if (output != null) {
